...
class OrchestratorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_find_defaults_to_short_window_and_explicit_limit_is_preserved(self):
        text = "# Section\n"+("x"*5000)+"needle"+("y"*70000)
        await self.orch._parse_cache.put("2401.00001v2", "", parsed(text))
        default = await self.orch.get_paper_text(document_id="2401.00001v2", find_text="needle")
        explicit = await self.orch.get_paper_text(document_id="2401.00001v2", find_text="needle", limit_chars=64000)
        ordinary = await self.orch.get_paper_text(document_id="2401.00001v2")
        self.assertEqual(len(default["markdown"]), 4000)
        self.assertEqual(len(explicit["markdown"]), 64000)
        self.assertEqual(len(ordinary["markdown"]), 64000)
        self.assertIn("needle", default["markdown"])
        self.assertEqual(default["match_status"], "matched")
        self.assertEqual(default["matches"], explicit["matches"])
        self.assertEqual(default["cost"]["model_calls"], 0)
        self.arxiv.resolve.assert_not_awaited()
        self.mineru.parse_pdf.assert_not_awaited()
    async def test_find_no_match_never_returns_full_paper_even_with_explicit_limit(self):
        await self.orch._parse_cache.put("2401.00001v2", "", parsed("Known paper text."))
        for limit in (None, 64000):
            result = await self.orch.get_paper_text(document_id="2401.00001v2", find_text="absent", limit_chars=limit)
            self.assertEqual(result["status"], "success")
            self.assertEqual(result["match_status"], "no_match")
            self.assertTrue(result["no_match"])
            self.assertEqual(result["markdown"], "")
            self.assertEqual(result["matches"], [])
            self.assertEqual(result["range"], {"start":0,"end":0})
            self.assertEqual(result["search_range"], {"start":0,"end":17})
            self.assertFalse(result["eof"])
            self.assertEqual(result["coverage"], "no_text_returned")
            self.assertEqual(result["cost"]["model_calls"], 0)
    async def test_find_before_requested_offset_is_not_a_match(self):
        await self.orch._parse_cache.put("2401.00001v2", "", parsed("needle before rest"))
        result = await self.orch.get_paper_text(document_id="2401.00001v2", find_text="needle", offset=7)
        self.assertTrue(result["no_match"])
        self.assertEqual(result["range"], {"start":7,"end":7})
